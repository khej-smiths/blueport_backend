...
export class AuthService {
  ...
  async login(input: LoginInputDto): Promise<string> {
    ...
    try {
      ...
      if (user.deletedAt) {
        throw new Error(ERR_DELETED_USER);
      }
      ...
      // 커스텀해서 만든 에러인 경우 그대로 전달
      if (e instanceof CustomGraphQLError) {
        e.message = `[login] - ${e.message}`;
        throw e;
      }

      if (e.message === ERR_DELETED_USER) {
      ...
      throw new CustomGraphQLError(e, {
        extensions: { code: 'UNEXPECTED_ERR' },
      });
    }
  }
}
